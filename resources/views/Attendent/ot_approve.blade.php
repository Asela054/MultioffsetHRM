    ...
    <script>
        $(document).ready(function () {
            ...
            function load_table() {
                ...
                $.ajax({
                    url: "{{ route('get_checked_ot_details') }}",
                    method: "POST",
                    data: {
                    ...
                    success: function (res) {
                        ...
                        if (ot_data.length > 0) {
                            ot_data.forEach(function (obj) {
                                let is_holiday = obj.is_holiday == 1 ? 'Yes' : 'No';

                                let from_input = '<input type="datetime-local" class="form-control form-control-sm" value="' + obj.from.replace(" ", "T") + '" readonly>';
                                let to_input = '<input type="datetime-local" class="form-control form-control-sm" value="' + obj.to.replace(" ", "T") + '" readonly>';
                            ...
                            var day_name = date.toLocaleDateString('en-US', options);
                                ...
                                ot_data_html += '<td>' + obj.emp_name_with_initial + ' - ' + obj.calling_name + '</td>';
                                ...
                                // ot_data_html += '<td>' + obj.dept_name + '</td>';
                                ot_data_html += '</tr>';
                            });
                        }
