...
@section('content')
                    ...
                    <div class="row">
                        ...
                        <div class="col-6">
                            ...
                            <div class="row">
                                ...
                                <div class="col-12">
                                    <span id="btn_message" class="fa-pull-right px-3" style="color: red"></span>
                                </div>
                            </div>
                        </div>
    ...
    <div class="modal fade" id="get_edit_attend_log_Modal" data-backdrop="static" data-keyboard="false" tabindex="-1"
        aria-labelledby="staticBackdropLabel" aria-hidden="true">
       <div class="modal-dialog modal-dialog-centered modal-md">
           <div class="modal-content">
               ...
               <div class="modal-body">
                   <div class="row">
                       <div class="col text-center">
                           <table id='edit_attendTable' class="table table-striped table-bordered table-sm small">
                               <thead>
                                   <tr>
...
@section('script')
    ...
    <script>
        $(document).ready(function () {
                ...
                if (emp_id != '' && month != '') {
                    $.ajax({
                        ...
                        success: function (res) {
                            ...
                            if (res.working_week_days_confirmed == null) {
                                $('#working_week_days').val(res.work_days);
                                // $('#working_week_days').css('border-color', 'red');
                                // $('#approvel').prop('disabled', true);
                                // $('#btn_message').text('Cannot Approve! Please Save Changes First..')
                            }
                            ...
                            $.each(attendances, function (key, value) {
                                ...
                                if (value.edit_status == 1) {
                                    $('#approvel').prop('disabled', true);
                                    $('#btn_message').text('Cannot Approve!..Please Approve Edit Attendance..')

                                    viewEditButton = '<td><button title="View Edit Logs" class="viewEditDetails btn btn-sm btn-secondary" data-editdetails=\'' + JSON.stringify(value.editdetails_array) + '\'><i class="fas fa-eye"></i></button></td>';
                                }
                                let tr =
                                    '<tr' + (value.edit_status == 1 ? ' style="background-color: yellow;"' : '') + '>' +
                                    '<td>' + value.date + '</td>' +
                            ...
                            $.each(working_work_days_breakdown, function (key, value) {
                                ...
                                let comment_input = '<textarea class="w-100 form-control-sm comment_input">' + is_no_pay_leave + '</textarea> ';
                                ...
                                tr +=
                                    '<td class="p-1">' + date_input + '</td>' +
                                    '<td class="p-1">' + value.day_name + '</td>' +
                                    '<td class="p-1">' + from_input + '</td>' +
                                    '<td class="p-1">' + to_input + '</td>' +
                                    '<td class="p-1">' + hours_input + '</td>' +
                                    '<td class="p-1">' + work_days_input + '</td>' +
                                    '<td class="p-1">' + comment_input + '</td>' +
                                    '</tr>';
            ...
            $(document).on('click', '.viewEditDetails', function (e) {
                var editdetails_array = $(this).data('editdetails');
                $('#edit_attendTable tbody').empty();
                ...
                for (var i = 0; i < maxLength; i++) {
                    ...
                    $('#edit_attendTable tbody').append(row);
                }
                $('#get_edit_attend_log_Modal').modal('show');
            });
