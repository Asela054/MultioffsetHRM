                        ...
                        <div class="form-row mb-1">
                            ...
                            <div class="col-md-3">
                                ...
                                <div class="input-group input-group-sm mb-3">
                                    <input type="datetime-local" id="from_datetime" name="from_datetime" class="form-control form-control-sm border-right-0" required>
...
<script src="https://unpkg.com/jspdf-autotable@3.8.3/dist/jspdf.plugin.autotable.js"></script>
    <script>
        $(document).ready(function () {
            ...
            let from_datetime = $('#from_datetime').val();
            let to_datetime = $('#to_datetime').val();
            let specialatten = $('input[name="specialattendance"]:checked').val();

            // load_dt('', '', '', from_date, to_date,specialatten);

            function load_dt(department, employee, location, from_datetime, to_datetime,specialatten) {
                ...
                $.ajax({
                    url: "{{ route('get_attendance_by_employee_data_time') }}",
                    method: "POST",
                    data: {
            ...
            $('#formFilter').on('submit',function(e) {
                ...
                load_dt(department, employee, location, from_datetime, to_datetime,specialatten);
            });
            ...
            async function download_excel(data) {
                ...
                let url = "{{ route('get_attendance_by_employee_data_excel_time') }}";
                $.ajax({
                    type: "POST",
                    url: url,
                    data: data,
...
async function generatePDF() {
    ...
    //     date = from_date + ' - ' + to_date;
    // }
    var from_val = $("#from_datetime").val();
    var to_val = $("#to_datetime").val();

    // 1. Convert input strings to Date Objects
    var d1 = new Date(from_val);
    var d2 = new Date(to_val);
    ...
    // 3. Format the dates (Example: 03/27/2026, 7:00 PM)
    var from_formatted = d1.toLocaleString('en-US', options);
    var to_formatted = d2.toLocaleString('en-US', options);

    // 4. Logic for the 'date' string
    var date = '';
    if (from_val === to_val) {
        date = from_formatted;
    } else {
        // If the dates are same but times are different, 
        // you might want to show the full range
        date = from_formatted + ' - ' + to_formatted;
    }
