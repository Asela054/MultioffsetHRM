...
class Attendance extends Model
{
    ...
    protected $fillable = ['emp_id', 'uid', 'state', 'timestamp', 'date',
        'approved', 'type', 'devicesno', 'location', 'edit_status', 'created_at', 'updated_at'];
    ...
    public function get_working_week_days($emp_id, $month)
    {
        ...
        foreach ($attendance as $att) {
            ...
            $diff = $work_hours_from->diffInMinutes($work_hours_to);
...
//            }
    ...
    public function get_ot_hours_by_date($emp_id, $off_time, $on_time, $record_date, $shift_start_, $shift_end_, $emp_department ){
        ...
        if($date_period == 0){
            ...
            if($day == 0){
                ...
                $ot_minutes = $ot_minutes - ($ot_minutes % 30);
    ...
    public function roundToNextHalfHour(Carbon $time): Carbon
    {
        ...
        // Special case: if minutes are exactly 30, return as-is (8:30 AM stays 8:30 AM)
        if ($minutes === 30) {
            return $roundedTime->second(0);
        }
        
        // Calculate the number of minutes past the last half-hour mark
        $minutes_past_30 = $minutes % 30;
        
        // Calculate minutes to add to reach the next half-hour mark
        if ($minutes_past_30 === 0) {
            // On the hour (0, 30, 60) - hour marks stay as-is
            // Actually, 0 minutes means we're exactly on the hour (8:00 AM)
            // According to your requirement: 8:00 AM to 8:29 AM round to 8:30 AM
            // So 8:00 AM should round to 8:30 AM
            $minutesToAdd = 30;
        } else {
            // Between minutes 1-29 or 31-59
    ...
    {
        $roundedTime = $time->copy()->second(0);
        $minutes = $roundedTime->minute;
        
        if ($minutes === 30) {
            // 5:30 PM -> 5:30 PM (no change)
            return $roundedTime;
        } elseif ($minutes > 30) {
            // 5:31 PM to 5:59 PM -> 5:30 PM
            return $roundedTime->subMinutes($minutes - 30);
        } else {
            // 0-29 minutes: 5:00 PM to 5:29 PM -> 5:00 PM
            // This includes 5:00 PM (minutes = 0) which should stay 5:00 PM
            return $roundedTime->subMinutes($minutes);
        }
    }
    // public function roundToPreviousHalfHour(Carbon $time): Carbon
    // {
    ...
    public function get_attendance_details($emp_id, $month)
    {
        ...
        foreach ($att_records as $att_record) {
            ...
            $otdatalist = DB::table('ot_approved')
                ...
                ->where('date', $record_date->format('Y-m-d')) 
                ->where('approved', 1)
                ->where('emp_id', $emp_id) 
                ->first();
            // dd($otdatalist);    
            ...
            // get edit status
            $get_edit_status = DB::table('attendances')
            ->join('attendance_edits', 'attendance_edits.attendance_id', '=', 'attendances.id')
            ->select('attendances.edit_status')
            ->where('attendances.emp_id', $emp_id)
            ->where('attendances.date', 'like', $att_record->date.'%')
            ->whereNull('attendances.deleted_at')
            ->where('attendances.edit_status', 1)
            ->exists();
            ...
            if ($get_edit_details->isNotEmpty()) {
                ...
                foreach ($get_edit_details as $detail) {
                    if ($detail->timestamp == $maxTimestamp && !is_null($detail->prev_val)) {
                        $lastdetails[] = $detail->prev_val;
                    }
                    if ($detail->timestamp == $minTimestamp && !is_null($detail->prev_val)) {
            ...
            $data = array(
                ...
                'normal_rate_otwork_hrs' => number_format($otdatalist->total_hours, 2),
                'double_rate_otwork_hrs' => number_format($otdatalist->total_double_hours, 2),
                'work_hours' => $work_hours,
                'edit_status' => $edit_status,
                'editdetails_array' => $editdetails_array,
            );
