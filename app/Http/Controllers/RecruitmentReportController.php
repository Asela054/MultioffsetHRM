...
class RecruitmentreportController extends Controller
{
    ...
    public function index()
    {
        ...
        $departments=DB::table('departments')->where('company_id', $company)->select('*')->get();
        $employees=DB::table('employees')->select('id','emp_name_with_initial')->where('emp_company', $company)->where('deleted',0)->where('employees.is_resigned', 0)->get();
