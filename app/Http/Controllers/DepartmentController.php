...
class DepartmentController extends Controller
{
    ...
    public function department_list_sel2(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $userDepartmentNames = $user->departments->pluck('name');

            $query = Department::where('name', 'LIKE', '%' . Input::get("term") . '%')
                ->where('company_id', $company);

            if (!$userDepartmentNames->isEmpty()) {
                $query->whereIn('name', $userDepartmentNames);
            }
            ...
            $countQuery = Department::where('company_id', $company);

            if (!$userDepartmentNames->isEmpty()) {
                $countQuery->whereIn('name', $userDepartmentNames);
            }
