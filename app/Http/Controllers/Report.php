...
//        }
    ...
    public function leave_report_list(Request $request)
    {
        ...
        $query2.= 'AND (employees.is_resigned = 0 OR (employees.is_resigned = 1 AND employees.resignation_date >= "'.$to_date.'")) ';
    ...
    public function employee_list_from_leaves_sel2(Request $request){
        if ($request->ajax())
        {
            $page = Input::get('page');
            $companyId = Session::get('company_id');
            $userDepartmentIds = Department::where('company_id', $companyId)->pluck('id');
            $resultCount = 25;
            $department = Input::get('department');
    ...
    public function no_pay_report_list_month(Request $request)
    {
        ...
        $monthDate = date('Y-m', strtotime($month));
        ...
        $emp_query .= " AND (employees.is_resigned = 0 OR 
                    (employees.is_resigned = 1 AND 
                    employees.resignation_date IS NOT NULL AND 
                    DATE_FORMAT(employees.resignation_date, '%Y-%m') >= '$monthDate'))";
